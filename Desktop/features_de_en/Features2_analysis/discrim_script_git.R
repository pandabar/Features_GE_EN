...
library(ggpubr)
setEPS()
tiff("Fig1.tiff", width = 4, height = 4, units = 'in', res = 300)
p<-ggboxplot(test, x="contrast", y="dprime", add="jitter", color="black")
ggpar(p, xlab = "Contrast", ylab = "Sensitivity (d')")
p
dev.off()

tiff("Fig2.tiff", width = 7, height = 4, units = 'in', res = 300)
rtplotdisc<-ggplot(discnoNA, aes(x=cont, y=rt, color=trialType)) + geom_jitter(aes(x=cont, y=rt), colour="grey", width=0.2) + geom_boxplot(outlier.shape = NA, alpha = 0.1) + xlab("Stimuli pair") + ylab("Reaction time (ms)") + theme_classic(base_size=12)
rtplotdisc
dev.off()